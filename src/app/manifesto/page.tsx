...
export default function ManifestoPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. The A-Shaped Scrolling Monologue
      ...
      sentences.forEach((sentence: HTMLElement) => {
        ...
        .fromTo(sentence, { opacity: 0 }, { opacity: 1, ease: "power1.inOut" })
        .to(sentence, { opacity: 0, ease: "power1.inOut" });
      });

      // 2. The Horizontal Scroll-Jacked Material Reveal
      const materialsSection = document.querySelector(".materials-section");
      const horizontalInner = document.querySelector(".materials-horizontal-inner");
      
      if (materialsSection && horizontalInner) {
  ...
  return (
    <main ref={containerRef} className="w-full relative overflow-x-hidden">
      ...
      <section className="relative w-full bg-brand-offWhite py-[50vh]">
        <div className="container mx-auto px-6 max-w-4xl flex flex-col gap-[100vh]">
          <p className="manifesto-sentence text-4xl md:text-6xl font-heading font-medium tracking-tight text-[#E0E0E0] uppercase leading-tight opacity-0">
      ...
      <section className="materials-section relative w-full h-screen bg-brand-black overflow-hidden">
        <div className="materials-horizontal-inner flex w-[300vw] h-full">
          {/* Layer 1 */}
          <div className="material-layer relative w-screen h-full flex-shrink-0">
            <Image src="https://drive.google.com/thumbnail?id=1BTEGqY0rTRruI-Ap4PPuB1KqHD6V7PAH&sz=w1000" alt="Fabric" fill className="object-cover grayscale" />
          ...
          {/* Layer 3 */}
          <div className="material-layer relative w-screen h-full flex-shrink-0">
            <Image src="https://drive.google.com/thumbnail?id=1iiHynhOYUmXC3udw7f5D6xMqcBDlgJqv&sz=w1000" alt="Hardware" fill className="object-cover grayscale" />
            <h2 className="absolute inset-0 flex justify-center items-center font-heading text-4xl md:text-7xl font-bold uppercase mix-blend-difference text-white">
              {"// "}THE BRUTALIST SILHOUETTE
            </h2>
          </div>
